	...
	<view class="modifygoods">
		<uni-forms ref="form" class="form" :modelValue="formData">
			<uni-forms-item label="库存地点" required :rules="[{'required': true,errorMessage: '必选项，请选择库存地址'}]"
				name="address">
				<uni-data-select class="form-select" :value="(this.goodsData.address.split('-')[1]) || ''"
			...
			<uni-forms-item label="一级分类" required :rules="[{'required': true,errorMessage: '必选项，请选择一级分类'}]"
				...
				<uni-data-select class="form-select" :value="(this.goodsData.firstClassification.split('-')[1]) || ''"
			...
			<uni-forms-item label="二级分类" required :rules="[{'required': true,errorMessage: '必选项，请选择二级分类'}]"
				...
				<uni-data-select class="form-select" :disabled="prohibit"
					:value="(this.goodsData.secondClassification.split('-')[1]) || ''"
...
<script>
	export default {
		onLoad(options) {
			this.goodsData = JSON.parse(options.params);
			this.number = this.goodsData.amount;
			this.formData = this.goodsData;
			...
			const digit = this.goodsData.firstClassification.split('-')[1];
			const value = this.goodsData.secondClassification.split('-')[1];
			this.secondClassificationList = this.firstClassificationList[digit].secondClassification;
			this.designationList = this.firstClassificationList[digit].secondClassification[value].designation;
		...
		methods: {
			...
			async _sendData(data) {
				const result = await this.$http.modify_goods(data);
				if (result) {
					uni.showToast({
						title: result.msg,
						icon: 'success'
...
<style lang="scss">
	@import '@/pages/modifygoods/css/modifygoods.scss';
</style>
